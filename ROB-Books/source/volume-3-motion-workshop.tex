...
The motor label visible in ROB's build photographs identifies a HengDrive B5685G family. Builder-owned outline drawings describe a 24 V motor/controller family with five external functions: power positive, power negative, speed feedback, direction, and PWM speed command. The drawings list several winding/gearbox options, while the builder recalls an early 12 V, roughly 80 rpm and 30 W arrangement. Those records do not yet identify which option is installed on each axis. Preserve the disagreement until a label, winding resistance, no-load test, and authorized manufacturer record resolve it.
...
The builder later installed a large 12-to-24 V boost converter and moved the motor supply to a 24 V bus. ROB immediately felt faster and stronger. The careful lesson is not ``double voltage always doubles torque.'' For a given motor, back-EMF and voltage strongly affect available speed, while torque follows current; the controller, converter, winding, gearing, battery sag, temperature, and load limit the result. The archived HengDrive family drawing is rated for 24 V, but the exact installed motor/controller option and converter margin must be verified before this observation becomes a specification.
...
\ROBPlaceholder{Confirm the exact B5685G winding, integrated controller, gearbox, five-wire polarity/logic levels, and installed sprocket pair on each tread and the flipper. Obtain publication permission for the archived manufacturer drawings and add measured 12 V/24 V no-load speed, loaded current, thermal rise, turning current on each surface, and flipper lift margin.}
