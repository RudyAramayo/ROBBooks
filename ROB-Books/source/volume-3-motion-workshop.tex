...
\begin{fieldnotebox}[title={EIGHT YEARS OF USE IS NOT A LOAD CASE}]
ROB has been revised and operated over roughly eight years, but the archive contains no verified ready-to-run mass, pose-dependent center of mass, arm payload, flipper load, slope limit, or transport load case. Those values cannot be inferred from survival or photographs. The force diagrams in this volume teach how a future measured redesign should reason; they do not rate ROB or a copy.
\end{fieldnotebox}
...
The purchased SuperDroid Robots LT2 tread system was ROB's one non-negotiable starting interface. The builder first mocked its envelope in cardboard, then planned maple-plywood parts around it and a flipper/third-wheel variation inspired by Johnny 5. That is systems engineering in a wonderfully practical form: freeze the one part you cannot change, represent it cheaply, and iterate everything that touches it.

The original LT2 is designed as a skid-steer tracked platform. SuperDroid's own support material warns through its payload discussion that added weight and high-traction treads make the sideways skid required for turning more difficult. ROB added arms, cameras, computers, an inverter, batteries, and structure far above the base. On concrete, the builder now often turns with small forward/back arcs like a car instead of demanding a stationary pivot. This is not merely a software inconvenience; it is the floor, tread contact patch, normal force, center of mass, gear ratio, motor current limit, and structure negotiating at once.
...
The motor label visible in ROB's build photographs identifies a HengDrive B5685G family. The builder recalls an early 12 V, roughly 80 rpm and 30 W arrangement, but the public evidence does not establish the installed winding, gearbox, controller interface, or present rating on each axis. Preserve those values as recollection until label photographs, non-destructive measurements, bounded tests, and an authorized public manufacturer record resolve the configuration.

The builder tried 16-, 32-, 40-, and 45-tooth sprockets while searching for a useful balance. A tooth count has no meaning by itself: the log must say whether it was the driver or driven sprocket and name its mate. Larger speed at the tread can reduce available output torque, while a reduction chosen for strong flipper lift can be painfully slow. ROB needed both: enough travel speed for a public robot and enough flipper torque to raise part of the heavy droid so the tracks could undulate over stairs.
...
\section*{What the 24 V boost changed}

The builder later installed a large 12-to-24 V boost converter and recalls moving the motor supply toward a 24 V bus. ROB immediately felt faster and stronger. The careful lesson is not ``double voltage always doubles torque.'' For a given motor, back-EMF and voltage strongly affect available speed, while torque follows current; the controller, converter, winding, gearing, battery sag, temperature, and load limit the result. The exact installed motor/controller option, commanded voltage, measured bus voltage, and converter margin are not established, so this observation remains history rather than an electrical specification.

\begin{missionbox}[title={BUILD A GEAR-RATIO TEST MATRIX}]
Make rows for 16, 32, 40, and 45 teeth. Add columns for driver teeth, driven teeth, calculated ratio, unloaded tread speed, current during a gentle turn, flipper lift success, motor temperature, and notes about surface. Leave a cell blank when it was not measured. The best ratio is the one that satisfies the declared mission with margin, not the one with the most dramatic single number.
\end{missionbox}

\begin{fieldnotebox}[title={THE UNDERPOWERED-MOTOR LESSON}]
The visible label identifies a HengDrive B5685G-family motor, while the author recalls an early roughly 30 W, 12 V choice that was too slow and underpowered for the growing robot. Sprocket changes recovered torque but did not deliver the intended speed; a later 12-to-24 V boost stage made ROB stronger and faster while it remained slow and high-torque. In hindsight the author would begin a redesign around at least roughly 120 W per drive motor and about four times the speed, but that is a design goal, not a drop-in specification. Exact winding, gearbox, controller logic, installed sprockets, current, temperature, and lift margin remain unmeasured. Private manufacturer drawings are excluded from the public edition.
\end{fieldnotebox}
...
\begin{fieldnotebox}[title={ACTUATOR ERAS, NOT A PARTS SCHEDULE}]
Photographs show multiple central, torso, third-wheel/flipper, hobby-servo, stepper, and AMBER-arm eras. They do not provide a dated model/voltage/stroke/force/duty-cycle inventory or prove which actuator remains installed. The historical record preserves the evolving roles; a reproduction requires a fresh label survey, limit/feedback inspection, load analysis, and safe-state test.
\end{fieldnotebox}
...
\begin{fieldnotebox}[title={WHY THERE IS NO DOWNLOADABLE ROB CAD}]
ROB was shaped by hand from cardboard, scrap wood, maple-plywood templates, fitted brackets, and later metal revisions. The final CAD, controlled templates, material/fastener schedule, fits, tolerances, and complete list of surviving wood parts do not exist. Reconstructing them from a measured scan is an explicit next project. Readers should reproduce the prototype-and-evidence method, not copy unmeasured geometry.
\end{fieldnotebox}
...
Machine selection, feeds and speeds, tool choice, workholding, chip control, coolant, guarding, ventilation, welding process, electrical protection, hot-work control, and post-weld inspection must be written by qualified adults for the actual shop. This volume does not supply those shop-specific procedures.
\end{safetybox}

\begin{fieldnotebox}[title={FABRICATION MEMORY IS NOT A SHOP TRAVELER}]
Rodolfo Aramayo identifies himself as the sole builder. The photo sequence records drilling, fitting, bracket and rail work, machining revisions, coating, wiring, and assembly, but it does not preserve machine parameters, workholding, welding procedures, PPE decisions, or inspection results. This volume therefore teaches how to plan and document fabrication without presenting an invented operation sequence.
\end{fieldnotebox}
...
\begin{safetybox}[title={NO QUANTIFIED PERFORMANCE OR SAFETY RATING}]
The author reports a slow, high-torque robot, difficult skid turns, outdoor/grass lessons, gearing changes, voltage conversion, and ambitious stair/flipper experiments. The archive contains no complete measured speed, stopping-distance, current, temperature, slope, lift, fastener, or failed-part report. Eight years of operation does not establish a safe limit. The seven-rung test ladder is a method for future evidence, not a claim that ROB passed it.
\end{safetybox}

\input{volume-3-deep-dive.tex}
